...
const translations = {
  ...
  en: {
    ...
    videoCount: 'Videos',
    linkCount: 'Links',
    selectChatPrompt: 'Please select chat to view',
    inputPlaceholder: 'Aa (Archive history view)',
...
function StartScreen({ openDirPicker }: { openDirPicker: () => void }) {
  const contents = [
    <div key='step-1' className='flex w-full flex-col items-center justify-center'>
    ...
    <div key='step-2' className='flex w-full flex-col items-center justify-center'>
      <img src='/images/step1.png' className='mb-5 w-full max-w-5xl' alt='step-1' />
    ...
    <div key='step-3' className='flex w-full flex-col items-center justify-center'>
      <img src='/images/step2.png' className='mb-5 w-full max-w-5xl' alt='step-2' />
      <h2 className='text-center text-xl font-bold'>
...
export default function HomePage() {
  ...
  const { windowControlsOverlayEnable, windowControlsOverlayRect } = useWindowOverlay();
    ...
    return (
      ...
      >
          ...
          {chats.length > 0 && (
            <Virtuoso
              ...
              itemContent={(index) => {
                ...
                return (
                  <div className='max-w-full cursor-default px-1 py-1.5' key={chat.dirName}>
                    <div
                      className={cx(
                        'flex items-center gap-2 rounded-lg py-3 px-5 hover:bg-gray-100 hover:dark:bg-gray-600',
                        {
                          'bg-gray-100 dark:bg-gray-600': folderName === chat.dirName,
                        }
                      )}
                      onClick={() => {
        ...
        <div className='flex flex-1 flex-col'>
          ...
          >
            <h3 className='select-none text-lg font-semibold'>
              {currentMessage ? decodeString(currentMessage.title) : 'Please select chat to view'}
            </h3>
            <button
              className='rounded-full border-none p-2 hover:bg-gray-100 hover:dark:bg-gray-600'
                  ...
                  .map(([senderName, count]) => (
                    <div key={senderName} className='flex justify-between'>
                      ...
                      <span className='ml-2 text-base text-gray-500'>
                        ({((count / currentMessage.messages.length) * 100).toFixed(1)} %) {count}
                      </span>
                    </div>
                  ))}
                <div className='flex justify-between'>
